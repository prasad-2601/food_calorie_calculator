...
```text
Application / Framework Preset: Python
```

If Python is not shown, choose `Other`. Do not choose `Services`.
...
The project is not Angular and it is not a Vercel Services project. If Vercel tries to run `ng build`, change the framework preset. If Vercel says the framework is set to `services`, change the framework preset to `Python` or `Other`, then redeploy.
